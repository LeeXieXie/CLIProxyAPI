...
func TestManagementUsageRequiresManagementAuthAndPopsArray(t *testing.T) {
	...
	if errUnmarshal := json.Unmarshal(statsRR.Body.Bytes(), &statsPayload); errUnmarshal != nil {
		t.Fatalf("unmarshal usage stats response: %v body=%s", errUnmarshal, statsRR.Body.String())
	}
	if statsPayload.Usage == nil {
		t.Fatalf("usage stats payload missing usage object: %s", statsRR.Body.String())
	}
